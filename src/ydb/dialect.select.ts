...
import {
  orderSelectedFields,
  type YdbSelectedFieldsOrdered,
} from "../ydb-core/result-mapping.js";
import type {
  YdbJoinConfig,
  YdbSelectConfig,
...
function yqlBindingName(alias: string): string {
  if (!/^[A-Za-z_][A-Za-z0-9_]*$/u.test(alias)) {
    throw new Error(
      `YDB CTE alias "${alias}" cannot be used as a YQL binding name`,
    );
  }
...
export function getSelectionAliases(
  fields: YdbSelectedFieldsOrdered,
): string[] {
...
export function buildJoins(
  ...
  const joinsSql = joins.map((join) => {
    ...
    if (
      is(join.table, Table) &&
      (join.table as any)[(Table as any).Symbol.IsAlias]
    ) {
...
export function buildAssumeOrderBy(
...
function buildMatchRecognize(
  matchRecognize: YdbSelectConfig["matchRecognize"],
): SQL | undefined {
  if (!matchRecognize) {
    return undefined;
  }
...
function buildWindows(windows: YdbWindowClause[] | undefined): SQL | undefined {
  ...
  return yql` window ${yql.join(
    windows.map(
      (window) => yql`${yql.identifier(window.name)} AS ${window.definition}`,
    ),
    yql`, `,
  )}`;
}
...
function buildSimpleSelectQuery(
  config: Omit<
    YdbSelectConfig,
    "table" | "fields" | "fieldsFlat" | "setOperators"
  > & {
    table?: unknown;
...
function buildEmulatedSetOperationQuery(
  ...
  const rightSelection = yql.join(
    selectionAliases.map(
      (alias) =>
        yql`${qualifyIdentifier(rightInputAlias, alias)} as ${yql.identifier(alias)}`,
    ),
    yql`, `,
  );
  ...
  const selection = yql.join(
    selectionAliases.map(
      (alias) =>
        yql`${qualifyIdentifier(leftAlias, alias)} as ${yql.identifier(alias)}`,
    ),
    yql`, `,
  );
  const joinSql =
