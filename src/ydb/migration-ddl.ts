...
export type YdbShowCreateObjectType =
  | "table"
  | "view"
  | "topic"
  | "async replication"
...
function getMigrationTableName(config: YdbMigrationTableConfig): string {
  ...
  return config.migrationsSchema
    ? `${config.migrationsSchema}/${tableName}`
    : tableName;
}

function getMigrationLockTableName(config: YdbMigrationTableConfig): string {
  const configuredTable =
    typeof config.migrationLock === "object"
      ? config.migrationLock.table
      : undefined;
  const tableName =
...
function getPrimaryKeyColumns(
  ...
  const inlinePrimaryKeys = columns.filter(
    (column) => (column as any).primary === true,
  );
...
function renderTableOptions(
  ...
  return Object.entries(options).map(
    ([key, value]) => `${key} = ${renderTableOptionValue(value)}`,
  );
}
...
function renderStatementOptions(
  ...
  return Object.entries(options).map(
    ([key, value]) => `${key} = ${renderStatementOptionValue(value)}`,
  );
}
...
function renderColumnFamilyAlterActions(
  name: string,
  options: YdbColumnFamilyOptions,
): string[] {
  ...
  if (options.data !== undefined) {
    rendered.push(
      `ALTER FAMILY ${familyName} SET DATA ${escapeDoubleQuoted(options.data)}`,
    );
  }
  if (options.compression !== undefined) {
    rendered.push(
...
function getColumnFamilyByColumnName(
  ...
  for (const family of columnFamilies) {
    if (usedFamilyNames.has(family.config.name)) {
      throw new Error(
        `YDB migrate() duplicate column family "${family.config.name}"`,
      );
    }
...
function renderPartitioning(
  ...
  if (partitioning.length > 1) {
    throw new Error(
      "YDB migrate() supports only one PARTITION BY definition per table",
    );
  }
...
function renderIndexConfig(config: YdbIndexConfig): string {
  ...
  fragments.push(
    `ON (${config.columns.map((column) => escapeName(column.name)).join(", ")})`,
  );
...
function renderAddIndexAction(index: YdbIndex | YdbUniqueConstraint): string {
  const rendered =
    "config" in index && "unique" in index.config
      ...
      : renderIndexConfig(
          uniqueConstraintToIndex(index as YdbUniqueConstraint),
        );
...
function uniqueConstraintToIndex(
  constraint: YdbUniqueConstraint,
): YdbIndexConfig {
  return {
...
function renderColumnsList(columns: readonly (string | YdbColumn)[]): string {
  return columns
    .map((column) =>
      escapeName(typeof column === "string" ? column : column.name),
    )
    .join(", ");
}
...
function renderTopicConsumer(consumer: YdbTopicConsumer): string {
  const settings = consumer.settings
    ? renderTableOptions(consumer.settings)
    : [];
...
function normalizeAlterAsyncReplicationOptions(
  ...
  if (options.failoverMode !== undefined)
    rendered["FAILOVER_MODE"] = options.failoverMode;
...
function normalizeTransferOptions(
  ...
  if (options.batchSizeBytes !== undefined)
    rendered["BATCH_SIZE_BYTES"] = options.batchSizeBytes;
...
function renderAccessPermissions(permissions: YdbAccessPermissions): string {
  ...
  return list
    .map((permission) => renderAccessPermission(permission))
    .join(", ");
}
...
function normalizeChangefeedOptions(
  ...
  if (options.initialScan !== undefined)
    rendered["INITIAL_SCAN"] = options.initialScan;
...
function renderChangefeedOptions(options: YdbChangefeedOptions): string {
  const renderedOptions = Object.entries(
    normalizeChangefeedOptions(options),
  ).map(([key, value]) => {
    if (isRawTableOptionValue(value)) {
    ...
    return `${key} = ${renderTableOptionValue(value)}`;
  });

  if (renderedOptions.length === 0) {
    throw new Error(
      "YDB migrate() CHANGEFEED WITH requires at least one option",
    );
  }
...
function renderAlterTableAction(action: YdbAlterTableAction): string[] {
  switch (action.kind) {
    ...
    case "set_table_options":
      return [
        `SET (${requireNonEmptyOptions(action.options, "ALTER TABLE SET").join(", ")})`,
      ];
    case "reset_table_options":
      return [`RESET (${action.names.join(", ")})`];
...
export function buildMigrationTableBootstrapSql(
...
export function buildMigrationLockTableBootstrapSql(
  config: YdbMigrationTableConfig = {},
): string {
...
export function buildCreateTableSql(
  ...
  const definitions = [
    ...
    ...columnFamilies.map((family) =>
      renderColumnFamilyDefinition(family.config),
    ),
  ...
  if (withOptions.length > 0) {
    parts.push(
      "WITH (",
      withOptions.map((option) => `  ${option}`).join(",\n"),
      ")",
    );
  }
...
export function buildAnalyzeSql(
  ...
  const columnSql =
    columns && columns.length > 0 ? ` (${renderColumnsList(columns)})` : "";
...
export function buildCreateTopicSql(
  name: string,
  options: YdbCreateTopicOptions = {},
): string {
  const consumers = (options.consumers ?? []).map((consumer) =>
    renderTopicConsumer(consumer),
  );
  const settings = renderTableOptions(options.settings ?? {});
  const consumersSql =
    consumers.length > 0 ? ` (\n  ${consumers.join(",\n  ")}\n)` : "";
  const settingsSql =
    settings.length > 0 ? ` WITH (\n  ${settings.join(",\n  ")}\n)` : "";
...
export function buildCreateAsyncReplicationSql(
  ...
  const targetSql = targets
    .map(
      (target) => `${escapeName(target.remote)} AS ${escapeName(target.local)}`,
    )
    .join(", ");
...
export function buildCreateTransferSql(
  ...
  const renderedOptions = renderStatementOptions(
    normalizeTransferOptions(options),
  );
  const withSql =
    renderedOptions.length > 0 ? ` WITH (${renderedOptions.join(", ")})` : "";
...
export function buildShowCreateSql(
  objectType: YdbShowCreateObjectType,
  name: string,
): string {
...
export function buildDropChangefeedSql(
  table: string | YdbTable,
  name: string,
): string {
...
export function buildMigrationSql(
  ...
  for (const operation of operations) {
    switch (operation.kind) {
      case "create_table":
        statements.push(
          buildCreateTableSql(operation.table, {
      ...
      case "drop_table":
        statements.push(
          buildDropTableSql(operation.table, { ifExists: operation.ifExists }),
        );
        break;
      case "analyze":
      ...
      case "create_view":
        statements.push(
          buildCreateViewSql(operation.name, operation.query, {
            ...operation.options,
            ifNotExists:
      ...
      case "drop_view":
        statements.push(
          buildDropViewSql(operation.name, { ifExists: operation.ifExists }),
        );
        break;
      case "create_topic":
      ...
      case "alter_async_replication":
        statements.push(
          buildAlterAsyncReplicationSql(operation.name, operation.options),
        );
        break;
      case "drop_async_replication":
        statements.push(
          buildDropAsyncReplicationSql(operation.name, {
      ...
      case "alter_transfer":
        statements.push(
          buildAlterTransferSql(operation.name, {
      ...
      case "drop_user":
        statements.push(
          buildDropUserSql(operation.names, { ifExists: operation.ifExists }),
        );
        break;
      case "create_group":
        statements.push(
          buildCreateGroupSql(operation.name, { users: operation.users }),
        );
        break;
      case "alter_group":
        statements.push(
      ...
      case "drop_group":
        statements.push(
          buildDropGroupSql(operation.names, { ifExists: operation.ifExists }),
        );
        break;
      case "grant":
        statements.push(
          buildGrantSql({
            ...
            on: operation.on,
            to: operation.to,
            withGrantOption: operation.withGrantOption,
          }),
        );
        break;
      case "revoke":
        statements.push(
          buildRevokeSql({
            ...
            on: operation.on,
            from: operation.from,
            grantOptionFor: operation.grantOptionFor,
          }),
        );
        break;
      case "show_create":
        statements.push(
          buildShowCreateSql(operation.objectType, operation.name),
        );
        break;
      case "add_columns":
        statements.push(
          ...buildAddColumnsSql(operation.table, operation.columns),
        );
        break;
      case "drop_columns":
        statements.push(
          ...buildDropColumnsSql(operation.table, operation.columns),
        );
        break;
      case "add_index":
      ...
      case "set_table_options":
        statements.push(
          buildAlterTableSetOptionsSql(operation.table, operation.options),
        );
        break;
      case "reset_table_options":
        statements.push(
          buildAlterTableResetOptionsSql(operation.table, operation.names),
        );
        break;
      case "add_column_family":
        statements.push(
          buildAddColumnFamilySql(operation.table, operation.family),
        );
        break;
      case "alter_column_family":
        statements.push(
      ...
      case "drop_changefeed":
        statements.push(
          buildDropChangefeedSql(operation.table, operation.name),
        );
        break;
      case "alter_table":
...
export function normalizeInlineMigration(
  ...
  if (sqlStatements.length === 0) {
    throw new Error(
      `YDB migrate() received migration #${index + 1} without sql or operations`,
    );
  }
  ...
  return {
    ...
    hash:
      migration.hash ?? crypto.createHash("sha256").update(text).digest("hex"),
    bps: migration.breakpoints ?? false,
    sql: sqlStatements,
  };
}

export function buildMigrationHistorySelectSql(
  config: YdbMigrationTableConfig = {},
): string {
  const migrationTableName = getMigrationTableName(config);
