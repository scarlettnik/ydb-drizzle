...
function getColumnNameAndConfig<TConfig extends Record<string, any> | undefined>(a: string | TConfig | undefined, b: TConfig | undefined) {
  return {
...
} & (T["notNull"] extends true ? { notNull: true } : {}) & (T["default"] extends true ? { hasDefault: true } : {});
...
> {
  ...
  override build<TTable extends Table>(table: TTable): YdbCustomColumn<MakeColumnConfig<T, TTable["_"]["name"]>> {
...
export class YdbCustomColumn<T extends ColumnBaseConfig<"custom", "YdbCustomColumn">> extends YdbColumn<T> {
  ...
  private readonly mapFrom?: (value: T["driverParam"]) => T["data"];
  ...
  override mapToDriverValue(value: T["data"]): T["driverParam"] {
    return typeof this.mapTo === "function" ? (this.mapTo(value) as T["driverParam"]) : (value as T["driverParam"]);
  }
}
...
export function customType<T extends CustomTypeValues = CustomTypeValues>(
  ...
  <TConfig extends Record<string, any> & T["config"]>(fieldConfig: TConfig): YdbCustomColumnBuilder<ConvertCustomConfig<"", T>>;
  <TName extends string>(dbName: TName, fieldConfig: T["config"]): YdbCustomColumnBuilder<ConvertCustomConfig<TName, T>>;
} : {
