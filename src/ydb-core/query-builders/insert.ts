...
import { Param, SQL, sql as yql, type SQL as SQLType } from "drizzle-orm/sql/sql";
...
import { getInsertColumnEntries, getPrimaryColumnKeys, getTableColumns, resolveInsertValue, validateTableColumnKeys } from "./utils.js";
...
function getProvidedColumnEntries(table: YdbTable, rows: InsertValues[], command: InsertCommand): Array<[string, YdbColumn]> {
...
function getSelectColumnEntries(
  ...
  if (selectedKeys.length === 0) {
    throw new Error("Insert select error: selected fields must include at least one table column");
  }
  ...
  for (const key of selectedKeys) {
    if (!(key in columns)) {
      throw new Error(`Insert select error: selected field "${key}" is not a column of the target table`);
    }

    if (!insertableColumns.has(key)) {
      throw new Error(`Insert select error: selected field "${key}" is not insertable in ${command}()`);
    }
  }
...
abstract class YdbInsertLikeBuilder<TResult = unknown> extends QueryPromise<TResult> {
  protected valuesData?: InsertValues | InsertValues[];
  protected selectQuery?: InsertSelectQuery;
  protected selectColumnEntries?: Array<[string, YdbColumn]>;
  protected returningFields?: YdbSelectedFieldsOrdered;
...
export class YdbInsertBuilder<TResult = unknown> extends YdbInsertLikeBuilder<TResult> {
  ...
  private buildOnDuplicateKeyUpdateQuery(rows: InsertValues[]): SQLType {
    ...
    const incomingSql = yql.join(
      rows.map((row) => yql`select ${
        yql.join(
          columnEntries.map(([key, column]) => yql`${resolveInsertValue(column, row[key])} as ${yql.identifier(column.name)}`),
          yql`, `,
        )
      }`),
      yql` union all `,
    ...
    const mergedSelections = yql.join(
      columnEntries.map(([key, column]) => {
        ...
        if (this.onDuplicateSet && key in this.onDuplicateSet) {
          return yql`case when ${conflictDetectedSql} is null then ${
    ...
    const joinSql = yql.join(
      primaryColumns.map((column) => yql`${column} = ${qualifyAlias(incomingAlias, column.name)}`),
      yql` and `,
    );
