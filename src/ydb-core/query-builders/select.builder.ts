...
export class YdbSelectBuilder<TResult = unknown[]> extends QueryPromise<TResult> implements YdbSetOperatorSource {
  ...
  private requireTable(): unknown | undefined {
    if (
      ...
      Object.keys(this.config.fields).length === 0
    ) {
  ...
  private getTargetConfigForTailClauses():
    ...
    return (
      this.config.setOperators[this.config.setOperators.length - 1] ??
      this.config
    );
  }
  ...
  private createJoin(joinType: YdbJoinType) {
    return (table: unknown, on?: SQL | ((fields: SelectFields) => SQL)) => {
      ...
      const resolvedOn =
        typeof on === "function"
          ? on(createSelectionProxy(this.config.fields, "sql"))
          : on;
  ...
  private createSetOperator(
    ...
    return (
      rightSelection:
        | YdbSetOperatorSource
        | ((
            operators: ReturnType<typeof getSetOperatorHelpers>,
          ) => YdbSetOperatorSource),
    ) => {
      const rightSelect =
    ...
    if (typeof columns[0] === "function") {
      const groupBy = columns[0](
        createSelectionProxy(this.config.fields, "alias"),
      );
    ...
    if (typeof columns[0] === "function") {
      ...
      (target as YdbSetOperatorConfig | typeof this.config).orderBy =
        Array.isArray(orderBy) ? orderBy : [orderBy];
      return this;
    }

    (target as YdbSetOperatorConfig | typeof this.config).orderBy =
      columns as SQLWrapper[];
    return this;
  }

    assumeOrderBy(
    ...columns: (string | SQLWrapper)[] | [(string | SQLWrapper)[]]
  ): this {
    ...
    this.config.assumeOrderBy = this.normalizeColumnExpressions(
      resolved,
      "assumeOrderBy()",
    );
    return this;
    ...
    const target = this.getTargetConfigForTailClauses();
    (target as YdbSetOperatorConfig | typeof this.config).limit =
      normalizeCountValue(limit, "limit");
    return this;
  }

    offset(offset: number): this {
    const target = this.getTargetConfigForTailClauses();
    (target as YdbSetOperatorConfig | typeof this.config).offset =
      normalizeCountValue(offset, "offset");
    return this;
  }
    ...
    if (this.isPartialSelect) {
      throw new Error(
        "YDB without() is only supported for whole-source select() queries",
      );
    }
    ...
    window(
    name: string,
    definition: YdbWindowDefinitionConfig | SQLWrapper,
  ): this {
    ...
    this.config.windows = [
      ...windows,
      { name, definition: windowDefinition(definition) },
    ];
    return this;
  }
    ...
    uniqueDistinct(
    ...hints: YdbUniqueDistinctHint[] | [YdbUniqueDistinctHint[]]
  ): this {
    ...
    if (this.config.distinctOn && this.config.distinctOn.length > 0) {
      throw new Error(
        "YDB select() cannot combine distinct() and distinctOn()",
      );
    }
    ...
    for (const setOperator of setOperators) {
      if (
        ...
        typeof (setOperator.rightSelect as any).markUsedInSetOperation ===
          "function"
      ) {
    ...
    toSQL() {
    const { typings: _typings, ...query } = this.dialect.sqlToQuery(
      this.getSQL(),
    );
    return query;
  }
    ...
    const joinsNotNullableMap =
      Object.keys(this.joinsNotNullableMap).length > 0
        ? this.joinsNotNullableMap
        : undefined;
    ...
    return session.prepareQuery<
      YdbPreparedQueryConfig & { execute: TResult; all: TResult }
    >(this.getSQL(), orderedFields, name, true, resultMapper as any);
  }
