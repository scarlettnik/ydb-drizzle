...
export type YdbValuesPrimitive =
  | string
  | number
  | boolean
  | bigint
...
export interface YdbMatchRecognizeConfig {
  ...
  orderBy?: SQLWrapper[];
  measures?: Readonly<Record<string, SQLWrapper>>;
  rowsPerMatch?: "ONE ROW PER MATCH" | "ALL ROWS PER MATCH";
  afterMatchSkip?: string;
  pattern: string | SQLWrapper;
  define?: Readonly<Record<string, SQLWrapper>>;
}
...
export interface YdbWindowDefinitionConfig {
  partitionBy?: readonly SQLWrapper[];
  orderBy?: readonly SQLWrapper[];
  frame?: string | SQLWrapper;
}

export type YdbGroupingSet = readonly SQLWrapper[];

export type YdbKnnDistanceFunction =
  | "CosineDistance"
  | "EuclideanDistance"
  | "ManhattanDistance";

export type YdbKnnSimilarityFunction =
  | "CosineSimilarity"
...
function normalizeRows(rows: readonly YdbValuesRow[]): {
  ...
  return {
    ...
    rows: rows.map((row) => {
      ...
      return yql`(${yql.join(
        columns.map((column) => renderValue(row[column] ?? null)),
        yql`, `,
      )})`;
    }),
  };
...
function renderExpressionList(values: readonly SQLWrapper[]): SQL {
  return yql.join(
    values.map((value) => yql`${value}`),
    yql`, `,
  );
}
...
export function valuesTable(
  rows: readonly YdbValuesRow[],
  options: YdbValuesOptions = {},
): SQL {
  const normalized = normalizeRows(rows);
...
export function matchRecognize(
  config: YdbMatchRecognizeConfig | SQLWrapper,
): SQL {
  ...
  const measures =
    config.measures && Object.keys(config.measures).length > 0
      ...
      : undefined;
  const rowsPerMatch = config.rowsPerMatch
    ? yql.raw(`${config.rowsPerMatch} `)
    : undefined;
...
export function renderUniqueDistinctHints(
  hints: readonly YdbUniqueDistinctHint[],
): SQL {
  if (hints.length === 0) {
...
export function windowDefinition(
  config: YdbWindowDefinitionConfig | SQLWrapper,
): SQL {
...
export function groupingSets(
  ...sets: [YdbGroupingSet, ...YdbGroupingSet[]]
): SQL {
...
export function knnEuclideanDistance(
  vector: SQLWrapper,
  target: SQLWrapper,
): SQL {
...
export function knnManhattanDistance(
  vector: SQLWrapper,
  target: SQLWrapper,
): SQL {
...
export function knnCosineSimilarity(
  vector: SQLWrapper,
  target: SQLWrapper,
): SQL {
...
export function knnInnerProductSimilarity(
  vector: SQLWrapper,
  target: SQLWrapper,
): SQL {
