...
export function validateSetBasedMutationSelection(
  ...
  for (const key of selectedKeys) {
    if (!(key in columns)) {
      throw new Error(`${label} error: selected field "${key}" is not a column of the target table`);
    }
  }
  ...
  for (const key of primaryKeys) {
    if (!selectedKeys.includes(key)) {
      throw new Error(`YDB ${operation}().on() requires primary key column "${key}" in selected fields`);
    }
  }
}
