...
export class YdbPreparedQuery<T extends YdbPreparedQueryConfig = YdbPreparedQueryConfig> {
  ...
  constructor(
    ...
    private readonly customResultMapper?: (rows: unknown[][], mapColumnValue?: (value: unknown) => unknown) => T["execute"],
  ) {}
...
export class YdbSession {
  ...
  async execute<T = unknown>(query: YdbQuerySource, options?: YdbExecuteOptions): Promise<T> {
    ...
    const prepared = this.prepareQuery<YdbPreparedQueryConfig & { execute: T }>(query, undefined, undefined, false);
  ...
  async all<T = unknown>(query: YdbQuerySource, options?: YdbExecuteOptions): Promise<T[]> {
    ...
    const prepared = this.prepareQuery<YdbPreparedQueryConfig & { all: T[] }>(query, undefined, undefined, false);
    if (options?.arrayMode === true) {
      return prepared.values() as Promise<T[]>;
    }
  ...
  async get<T = unknown>(query: YdbQuerySource): Promise<T> {
    ...
    return this.prepareQuery<YdbPreparedQueryConfig & { get: T }>(query, undefined, undefined, false).get();
  }
