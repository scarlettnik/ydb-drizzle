...
interface YdbIndexBuilderDefaults {
  readonly indexType?: string;
  readonly withOptions?: YdbIndexWithOptions;
}

const vectorKMeansTreeIndexType = "vector_kmeans_tree";
const vectorTypes: readonly YdbVectorType[] = ["float", "uint8", "int8"];
const vectorDistances: readonly YdbVectorDistance[] = [
  "cosine",
  "manhattan",
  "euclidean",
];
const vectorSimilarities: readonly YdbVectorSimilarity[] = [
  "inner_product",
  "cosine",
];
...
function assertIntegerRange(
  ...
  if (!Number.isInteger(value) || value < min || value > max) {
    throw new Error(
      `YDB vector index ${name} must be an integer between ${min} and ${max}`,
    );
  }
}
...
function assertOneOf<T extends string>(
  ...
  if (!(allowed as readonly string[]).includes(value)) {
    throw new Error(
      `YDB vector index ${name} must be one of: ${allowed.join(", ")}`,
    );
  }
}
...
function normalizeVectorKMeansTreeOptions(
  ...
  if ((options.distance === undefined) === (options.similarity === undefined)) {
    throw new Error(
      "YDB vector index requires exactly one of distance or similarity",
    );
  }
  ...
  if (options.clusters ** options.levels > 1_073_741_824) {
    throw new Error(
      "YDB vector index clusters ** levels must be no more than 1073741824",
    );
  }

  if (options.vectorDimension * options.clusters > 4_194_304) {
    throw new Error(
      "YDB vector index vectorDimension * clusters must be no more than 4194304",
    );
  }
...
export class YdbIndexBuilder {
  ...
  constructor(
    ...
    return this.using(vectorKMeansTreeIndexType).with(
      normalizeVectorKMeansTreeOptions(options),
    );
  }
