...
export type AnyYdbTable<TPartial extends Partial<TableConfig> = {}> = YdbTable<UpdateTableConfig<TableConfig, TPartial>>;

export type YdbBuildColumn<TTableName extends string, TBuilder extends YdbColumnBuilderBase> = YdbColumn<
  MakeColumnConfig<TBuilder["_"], TTableName>,
  {},
  Simplify<Omit<TBuilder["_"], keyof MakeColumnConfig<TBuilder["_"], TTableName> | "brand" | "dialect">>
>;

export type YdbBuildColumns<TTableName extends string, TColumnsMap extends Record<string, YdbColumnBuilderBase>> = {
  [Key in keyof TColumnsMap]: YdbBuildColumn<TTableName, {
    _: Omit<TColumnsMap[Key]["_"], "name"> & {
      name: TColumnsMap[Key]["_"]["name"] extends "" ? Assume<Key, string> : TColumnsMap[Key]["_"]["name"];
    };
  }>;
};
...
export type YdbColumnsFactory<TColumnsMap extends YdbColumnsMap = YdbColumnsMap> = (builders: YdbColumnBuilders) => TColumnsMap;
export type YdbColumnsInput<TColumnsMap extends YdbColumnsMap = YdbColumnsMap> = TColumnsMap | YdbColumnsFactory<TColumnsMap>;
...
>(
  ...
  extraConfig:
    | ((self: YdbBuildColumns<TTableName, TColumnsMap>) => YdbTableExtraConfigValue[] | YdbTableExtraConfig)
    | undefined,
  schema?: TSchemaName,
  baseName = name,
): YdbTableWithColumns<{
  ...
  const parsedColumns = (typeof columns === "function" ? columns(getYdbColumnBuilders()) : columns) as TColumnsMap;
...
export const ydbTable: YdbTableFn = ((name: string, columns: YdbColumnsInput, extraConfig?: unknown) =>
...
export function ydbTableCreator(customizeTableName: (name: string) => string): YdbTableFn {
  return ((name: string, columns: YdbColumnsInput, extraConfig?: (self: Record<string, YdbColumn>) => YdbTableExtraConfigValue[] | YdbTableExtraConfig) =>
    ydbTableBase(customizeTableName(name), columns as any, extraConfig as any, undefined, name)) as YdbTableFn;
}
