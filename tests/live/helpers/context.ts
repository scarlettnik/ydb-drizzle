...
export function createLiveContext(): LiveTestContext {
  ...
  const uniqueSeed = Number((process.hrtime.bigint() + BigInt(Math.floor(Math.random() * 1_000_000))) % 1_000_000_000n);
  const baseIntId = uniqueSeed;
  const baseUint64Id = 2_000_000_000n + BigInt(uniqueSeed % 1_000_000);
  ...
  return {
    ...
    describeDbChange(t: TestContext, description: string): void {
      t.diagnostic(`DB change: ${description}`);
    },
    deleteUserRows,
    deletePostRows,
    deleteTypeRows,
