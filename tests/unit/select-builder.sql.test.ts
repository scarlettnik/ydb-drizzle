...
test("select sql", () => {
  const query = toQuery(
    new YdbSelectBuilder(session).from(users).where(eq(users.id, 7)),
  );
...
test("select without from sql", () => {
  const query = toQuery(
    new YdbSelectBuilder(session, { value: yql<number>`${1}` }),
  );
...
test("YDB SELECT source helpers render AS_TABLE, VALUES, and top-level VALUES", () => {
  const asTableQuery = toQuery(
    new YdbSelectBuilder(session, {
      id: yql`${yql.identifier("r")}.${yql.identifier("id")}`,
    }).from(asTable("$rows", "r")),
  );
  const fromAsTableQuery = toQuery(
    new YdbSelectBuilder(session, {
      id: yql`${yql.identifier("r")}.${yql.identifier("id")}`,
    }).fromAsTable("rows", "r"),
  );
  const fromValuesQuery = toQuery(
    new YdbSelectBuilder(session, {
  ...
  const topLevelValues = dialect.sqlToQuery(
    values([
      { id: 1, name: "one" },
      { id: 2, name: "two" },
    ]),
  ...
  assert.equal(
    fromAsTableQuery.sql,
    "select `r`.`id` from AS_TABLE($rows) as `r`",
  );
  assert.equal(
    fromValuesQuery.sql,
...
test("YDB SELECT gaps render WITHOUT, FLATTEN, SAMPLE, TABLESAMPLE, and MATCH_RECOGNIZE", () => {
  const withoutQuery = toQuery(
    new YdbSelectBuilder(session)
      .from(users)
      .without(users.name)
      .where(eq(users.id, 7)),
  );
  const flattenQuery = toQuery(
  ...
  const sampleQuery = toQuery(
    new YdbSelectBuilder(session, { id: users.id }).from(users).sample(0.25),
  );
  const tableSampleQuery = toQuery(
    new YdbSelectBuilder(session, { id: users.id })
      .from(users)
      .tableSample("bernoulli", 10, 7),
  );
  const matchRecognizeQuery = toQuery(
  ...
  assert.equal(
    withoutQuery.sql,
    "select * WITHOUT `name` from `users` where `users`.`id` = $p0",
  );
...
test("YDB SELECT renders unique/distinct hints, ASSUME ORDER BY, WINDOW, INTO RESULT, and advanced GROUP BY", () => {
  ...
  const sessionWindowQuery = toQuery(
    new YdbSelectBuilder(session, {
      ...
      .from(users)
      .groupBy(
        groupKey(sessionWindow(users.createdAt, "PT1H"), "session_start"),
      ),
  );
  const hopQuery = toQuery(
    new YdbSelectBuilder(session, {
      hopStart: hopStart(),
      hopEnd: hopEnd(),
      count: yql`count(*)`,
  ...
  const intoResultQuery = toQuery(
    new YdbSelectBuilder(session, { id: users.id })
      .from(users)
      .limit(1)
      .intoResult("selected_users"),
  );
  const knnQuery = toQuery(
    new YdbSelectBuilder(session, {
      similarity: knnCosineSimilarity(
        yql.identifier("embedding"),
        yql.raw("$target"),
      ),
    })
...
test("YDB script helpers render PRAGMA, DECLARE, ACTION, COMMIT, and INTO RESULT", () => {
  const scriptQuery = dialect.sqlToQuery(
    yqlScript(
  ...
  const intoResultQuery = dialect.sqlToQuery(
    intoResult(
  ...
  assert.equal(
    intoResultQuery.sql,
    "select `users`.`id` from `users` INTO RESULT `Result name`;",
  );
});
...
test("distinctOn and set operators sql", () => {
  ...
  const unionQuery = toQuery(
    unionAll(
      new YdbSelectBuilder(session, { value: users.name })
        .from(users)
        .where(eq(users.id, 1)),
  ...
  assert.ok(
    variadicDistinctOnQuery.sql.includes(
      "partition by `posts`.`user_id`, `posts`.`title`",
    ),
  );
  assert.equal(
    unionQuery.sql,
