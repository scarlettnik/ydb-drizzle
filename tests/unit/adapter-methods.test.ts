...
import { drizzle, integer, primaryKey, text, ydbTable } from "../../src/index.js";
...
test("upsert and replace select support partial target columns", () => {
  const upsertQuery = dialect.sqlToQuery(
    drizzle({
      async execute() {
        return { rows: [] };
      },
  ...
  const replaceQuery = dialect.sqlToQuery(
    drizzle({
      async execute() {
        return { rows: [] };
      },
  ...
  assert.equal(upsertQuery.sql, "upsert into `users` (`id`, `name`) select $p0 as `id`, `users`.`name` from `users`");
  assert.equal(replaceQuery.sql, "replace into `users` (`id`, `name`) select $p0 as `id`, `users`.`name` from `users`");
});
...
test("onDuplicateKeyUpdate supports table-level primary keys", () => {
  ...
  assert.ok(query.sql.includes("left join `keyed_users` on `keyed_users`.`id` = `__ydb_incoming`.`id`"));
  assert.deepEqual(query.params, [1, "insert value", "updated value"]);
});
