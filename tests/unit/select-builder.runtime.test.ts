...
test("select builder maps joins and advanced queries", async () => {
  ...
  const advancedSession = {
    prepareQuery(
      ...
      return {
        ...
        async execute() {
          const rows = built.sql.includes("__ydb_row_number")
            ? [[1, "Zecora"], [2, "Rainbow Dash"]]
            : [
  ...
  const joinedRows = await new YdbSelectBuilder(advancedSession)
    .from(users)
    .leftJoin(posts, eq(users.id, posts.userId))
    .orderBy(users.id, posts.id)
    .execute() as Array<Record<string, unknown>>;

  const distinctOnRows = await new YdbSelectBuilder(advancedSession, { userId: posts.userId, title: posts.title })
    .from(posts)
  ...
  assert.match(executedSql[0] ?? "", /^select `users`\.`id` as `__ydb_f0`, `users`\.`name` as `__ydb_f1`, `users`\.`created_at` as `__ydb_f2`, `users`\.`updated_at` as `__ydb_f3`, `posts`\.`id` as `__ydb_f4`, `posts`\.`user_id` as `__ydb_f5`, `posts`\.`title` as `__ydb_f6` from `users` left join `posts` on `users`\.`id` = `posts`\.`user_id` order by `users`\.`id`, `posts`\.`id`$/);
