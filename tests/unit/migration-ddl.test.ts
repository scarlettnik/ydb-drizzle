...
test("migration DDL generates create table with inline indexes and unique constraints", () => {
  ...
  assert.match(
    ddl,
    /INDEX `migration_users_name_idx` GLOBAL SYNC ON \(`name`\) COVER \(`age`\)/u,
  );
  assert.match(
    ddl,
    /INDEX `migration_users_name_unique` GLOBAL UNIQUE SYNC ON \(`name`\)/u,
  );
  assert.match(ddl, /PRIMARY KEY \(`id`\)/u);
});

test("migration DDL generates table options, partitioning, TTL, and column families", () => {
  const events = ydbTable(
    "migration_events",
    {
      id: integer("id").notNull().primaryKey(),
    ...
    (table) => [
      columnFamily("cold", { data: "rot", compression: "lz4" }).columns(
        table.payload,
      ),
...
test("migration DDL generates vector k-means tree indexes", () => {
  ...
  const ddl = buildCreateTableSql(articles);
  const similarityIndex = vectorIndex({
    ...
    .build(articles);
  const methodIndex = index("emb_euclidean_idx")
    .on(articles.embedding)
    .vectorKMeansTree({
      distance: "euclidean",
      vectorType: "int8",
...
test("migration DDL generates ALTER statements for table options and column families", () => {
  ...
  assert.equal(
    buildAlterTableResetOptionsSql(events, [
      "TTL",
...
test("migration DDL generates ANALYZE, VIEW, TOPIC, CHANGEFEED, rename, and multi-action ALTER TABLE", () => {
  const users = ydbTable("migration_admin_users", {
    id: integer("id").notNull().primaryKey(),
    age: integer("age"),
    name: text("name"),
    status: text("status"),
  });
  const ageIndex = index("migration_admin_users_age_idx")
    .on(users.age)
    .build(users);
  ...
  assert.equal(
    buildDropTopicSql("migration_admin_topic"),
  ...
  assert.equal(
    buildRenameTableSql(
      "migration_admin_users",
      "migration_admin_users_archive",
    ),
...
test("migration DDL generates temporary tables, replication, transfer, secrets, users, groups, grants, and SHOW CREATE", () => {
  const tempTable = ydbTable("migration_temp_events", {
    id: integer("id").notNull().primaryKey(),
  ...
  assert.equal(
    buildDropTransferSql("orders_transfer"),
  ...
  assert.equal(
    buildShowCreateSql("table", "orders"),
...
test("migration DDL rejects invalid YDB constructs", () => {
  ...
  assert.throws(
    () => buildAddIndexSql(users, uniqueAge),
    /cannot add UNIQUE indexes/u,
  );
});

test("migration DDL rejects invalid table option and family definitions", () => {
  const duplicateOptionTable = ydbTable(
    "duplicate_options",
    {
      id: integer("id").notNull().primaryKey(),
    },
  ...
  const duplicateTtlTable = ydbTable(
    "duplicate_ttl",
    {
      id: integer("id").notNull().primaryKey(),
      expiresAt: uint32("expires_at").notNull(),
  ...
  const duplicateFamilyTable = ydbTable(
    "duplicate_family",
    {
      id: integer("id").notNull().primaryKey(),
      payload: text("payload"),
  ...
  const duplicateFamilyNameTable = ydbTable(
  ...
  assert.throws(
    () => buildCreateTableSql(duplicateTtlTable),
    /supports only one TTL/u,
  );
  assert.throws(
...
test("migration DDL rejects invalid vector indexes", () => {
  const articles = ydbTable("user_articles", {
...
test("migration DDL includes inline column unique constraints", () => {
  const users = ydbTable("migration_unique_users", {
    ...
    externalId: text("external_id").unique(
      "migration_unique_users_external_unique",
    ),
  });
