...
test("builders", async () => {
  ...
  const upsertBuilder = new YdbUpsertBuilder(users, session).values({ id: 2, name: "Starlight" });
  const replaceBuilder = new YdbReplaceBuilder(users, session).values({ id: 3, name: "Fluttershy" });
  ...
  assert.equal(replaceBuilder.toSQL().sql, "replace into `users` (`id`, `name`, `created_at`, `updated_at`) values ($p0, $p1, $p2, $p3)");
  ...
  const executedUpsert = await upsertBuilder.execute() as unknown as { prepared: string };
  const executedReplace = await replaceBuilder.execute() as unknown as { prepared: string };
...
test("builders reject invalid state", () => {
  ...
  assert.throws(
    () => new YdbUpsertBuilder(users, session).getSQL(),
    /Upsert values are missing/,
  );
  assert.throws(
