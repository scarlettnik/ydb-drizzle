...
import {
  ...
  YdbDeleteBuilder,
} from "../../src/ydb-core/query-builders/delete.js";
import {
  YdbInsertBuilder,
  YdbReplaceBuilder,
  YdbUpsertBuilder,
} from "../../src/ydb-core/query-builders/insert.js";
import { YdbQueryBuilder } from "../../src/ydb-core/query-builders/query-builder.js";
import {
  YdbRelationalQuery,
  YdbRelationalQueryBuilder,
} from "../../src/ydb-core/query-builders/query.js";
import {
  except,
  intersect,
  union,
  unionAll,
  YdbSelectBuilder,
} from "../../src/ydb-core/query-builders/select.js";
import {
  YdbBatchUpdateBuilder,
  YdbUpdateBuilder,
} from "../../src/ydb-core/query-builders/update.js";
...
import {
  ...
  tableOptions,
  ttl,
} from "../../src/ydb-core/table-options.js";
...
import {
  ...
  valuesTable,
  windowDefinition,
} from "../../src/ydb-core/query-builders/select-syntax.js";
import {
  commit,
  declareParam,
  defineAction,
  ...
  pragma,
  yqlScript,
} from "../../src/ydb-core/query-builders/yql-script.js";
...
test("root public API re-exports runtime entry points", () => {
  ...
  assert.equal(
    publicApi.buildAlterAsyncReplicationSql,
    buildAlterAsyncReplicationSql,
  );
  ...
  assert.equal(
    publicApi.buildAlterColumnSetFamilySql,
    buildAlterColumnSetFamilySql,
  );
  assert.equal(
    publicApi.buildAlterTableResetOptionsSql,
    buildAlterTableResetOptionsSql,
  );
  assert.equal(
    publicApi.buildAlterTableSetOptionsSql,
    buildAlterTableSetOptionsSql,
  );
  ...
  assert.equal(
    publicApi.buildCreateAsyncReplicationSql,
    buildCreateAsyncReplicationSql,
  );
  ...
  assert.equal(
    publicApi.buildDropAsyncReplicationSql,
    buildDropAsyncReplicationSql,
  );
  ...
  assert.equal(
    publicApi.buildMigrationLockTableBootstrapSql,
    buildMigrationLockTableBootstrapSql,
  );
...
test("query builder barrel re-exports concrete builder implementations", () => {
  ...
  assert.equal(
    queryBuilders.YdbRelationalQueryBuilder,
    YdbRelationalQueryBuilder,
  );
  ...
  assert.equal(
    queryBuilders.knnInnerProductSimilarity,
    knnInnerProductSimilarity,
  );
