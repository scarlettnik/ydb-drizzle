...
test("delete on validates target columns and primary key", () => {
  ...
  assert.throws(
    () => new YdbDeleteBuilder(keyedUsers, session, dialect).on((qb) => qb.select({
...
test("insert returning", () => {
  const allColumns = toQuery(
    new YdbInsertBuilder(users, session)
      .values({ id: 1, name: "Pinkie Pie" })
      .returning(),
  );
...
test("native replace values", () => {
  const query = toQuery(
    new YdbReplaceBuilder(users, session).values({ id: 1, name: "Fluttershy" }),
  );
...
test("update on validates target columns and primary key", () => {
  ...
  assert.throws(
    () => new YdbUpdateBuilder(keyedUsers, session, dialect).on((qb) => qb.select({
      name: keyedUsers.name,
    }).from(keyedUsers)),
    /requires primary key column "id"/,
  );
...
test("batch update and delete sql", () => {
  const updateQuery = toQuery(
    new YdbBatchUpdateBuilder(users, session)
      .set({ name: "Applejack" })
      .where(eq(users.id, 1)),
  );
  const deleteQuery = toQuery(
    new YdbBatchDeleteBuilder(users, session)
      .where(eq(users.id, 2)),
  );
...
test("batch update and delete reject unsupported mutation clauses", () => {
  assert.throws(
    () => new YdbBatchUpdateBuilder(users, session).returning(),
    /not supported/,
  );
  assert.throws(
    () => new YdbBatchUpdateBuilder(users, session).on(),
    /not supported/,
  );
  assert.throws(
    () => new YdbBatchDeleteBuilder(users, session).returning(),
    /not supported/,
  );
  assert.throws(
    () => new YdbBatchDeleteBuilder(users, session).using(),
    /not supported/,
  );
  assert.throws(
    () => new YdbBatchDeleteBuilder(users, session).on(),
    /not supported/,
  );
});
